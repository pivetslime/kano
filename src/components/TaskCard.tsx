...
export function TaskCard({ task, users, onClick, className = '' }: TaskCardProps) {
  ...
  const handleDeleteClick = (e: React.MouseEvent) => {
    ...
    // Проверка, может ли пользователь удалить эту задачу
  ...
  // Получение последнего комментария для отображения
  ...
  const formatDescription = (text: string) => {
    ...
    // Простое форматирование текста - удаление markdown/HTML и обрезка
    let formatted = text
      .replace(/\*\*(.*?)\*\*/g, '$1') // Удаление жирного markdown
      .replace(/\*(.*?)\*/g, '$1') // Удаление курсивного markdown
      .replace(/<[^>]*>/g, '') // Удаление HTML тегов
      .replace(/^\s*[•\-\*]\s*/gm, '') // Удаление маркеров списка
      .replace(/^\s*\d+\.\s*/gm, '') // Удаление нумерованных списков
      .trim();
