...
interface AppContextType extends AppState {
  ...
  joinBoardByLink: (boardLink: string, userId: string) => Promise<boolean>;
  generateBoardLink: (boardId: string) => string;
  getBoardByLink: (boardLink: string) => Board | null;
}
...
export function AppProvider({ children }: { children: React.ReactNode }) {
      ...
      const demoBoards: Board[] = [
        {
          ...
          createdBy: '1',
          members: ['1', '2'], // Добавляем участников
          createdAt: new Date().toISOString(),
  ...
  const getBoardByLink = (boardLink: string): Board | null => {
    const boardId = boardLink.split('/board/')[1];
    return state.boards.find(board => board.id === boardId) || null;
  };

  // Присоединение к доске по ссылке
  const joinBoardByLink = async (boardLink: string, userId: string): Promise<boolean> => {
    const board = getBoardByLink(boardLink);
    if (!board) return false;

    // Добавляем пользователя к участникам доски, если его там нет
    ...
    // Устанавливаем эту доску как текущую
  ...
  const login = async (email: string, password: string, boardLink?: string): Promise<boolean> => {
    ...
    if (user) {
      ...
      if (boardLink) {
        const joinSuccess = await joinBoardByLink(boardLink, user.id);
        if (joinSuccess) {
          return true;
        }
      }
      
      // Проверка доступа пользователя к доскам
  ...
  const register = async (email: string, password: string, name: string, boardLink?: string): Promise<boolean> => {
    ...
    // Если предоставлена ссылка на доску, попытаться присоединиться
    if (boardLink) {
      const joinSuccess = await joinBoardByLink(boardLink, newUser.id);
      if (joinSuccess) {
        return true;
      }
    }
    
    // Создание персональной доски для нового пользователя
    const newBoard: Board = {
  ...
  const addBoard = (boardData: Omit<Board, 'id' | 'createdAt' | 'updatedAt'>) => {
    const newBoard: Board = {
      ...
      id: Date.now().toString(),
      members: [boardData.createdBy], // Создатель автоматически становится участником
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
    dispatch({ type: 'ADD_BOARD', payload: newBoard });
  ...
  const deleteBoard = (boardId: string) => {
    // Проверка, может ли пользователь удалить эту доску
    ...
    // Переключение на другую доску, если текущая была удалена
